...
class _ChatScreenState extends State<ChatScreen> {
  ...
  static const _quickReactions = <String, IconData>{
    '\u2764\uFE0F': Icons.favorite_rounded,
    '\u{1F44D}': Icons.thumb_up_alt_rounded,
    '\u{1F606}': Icons.sentiment_very_satisfied_rounded,
    '\u{1F62E}': Icons.sentiment_satisfied_alt_rounded,
    '\u{1F622}': Icons.sentiment_dissatisfied_rounded,
  };
  ...
  static const _safetyPresets = [
    'Mình đã đến nơi an toàn',
    'Mình đang trên đường về nhà',
    'Rảnh thì gọi lại cho mình nhé',
    'Mình đang gặp chuyện, để ý điện thoại giúp mình',
  ];
  ...
  IconData _reactionIcon(String reaction) =>
      _quickReactions[reaction] ?? Icons.add_reaction_outlined;
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                'Gửi 1 chạm để báo tình trạng cho cả nhà',
                style: GoogleFonts.inter(
                  fontSize: 12,
  ...
  void _showMessageMenu(ChatMessage m) {
    ...
    final myEmojis = m.reactions
        .where((r) => r.userId == _myUserId)
        .map((r) => r.emoji)
        .toSet();
      ...
      builder: (ctx) => SafeArea(
        child: Column(
          ...
          children: [
            ...
            // Hàng reaction nhanh — bấm reaction đã thả của mình để bỏ
            Row(
              mainAxisAlignment: MainAxisAlignment.center,
              children: _quickReactions.entries.map((entry) {
                final reaction = entry.key;
                final mine = myEmojis.contains(reaction);
                return GestureDetector(
                  onTap: () async {
                    Navigator.pop(ctx);
                    try {
                      mine
                          ? await chat.unreact(m.id, reaction)
                          : await chat.react(m.id, reaction);
                    } catch (err) {
                      _snackErr(err);
                    }
                  },
            ...
            ListTile(
              ...
              title: Text(
                m.isPinned ? 'Bỏ ghim' : 'Ghim tin nhắn',
              ...
              onTap: () async {
                ...
                try {
                  m.isPinned
                      ? await chat.unpinMessage(m.id)
                      : await chat.pinMessage(m.id);
                } catch (err) {
                  _snackErr(err);
                }
              },
                  ...
                  onTap: () {
                    ...
                    _showPickMemberSheet(
                      ...
                      onPick: (memberId) async {
                        try {
                          await context.read<ChatProvider>().createPrivate(
                            memberId,
                          );
                        } catch (e) {
                          _snackErr(e);
            ...
            children: [
              ...
              TextField(
                ...
                decoration: const InputDecoration(
                  hintText: 'Tên nhóm (VD: Hội bàn việc Tết)',
                ),
              ),
              const SizedBox(height: 12),
              Text(
                ...
                child: ListView(
                  ...
                  children: members.map((m) {
                    ...
                    return CheckboxListTile(
                      ...
                      onChanged: (_) => setS(
                        () => on ? selected.remove(m.id) : selected.add(m.id),
                      ),
                    );
                  }).toList(),
                ),
  ...
  void _showPickMemberSheet({
    required String title,
    required Future<void> Function(String memberId) onPick,
  }) {
    final members = context
        .read<FamilyProvider>()
        .members
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                '${conv.participants.length} thành viên · ${conv.type == 'PRIVATE' ? 'Chat riêng' : 'Nhóm'}',
                style: GoogleFonts.inter(
                  fontSize: 12,
                  color: AppColors.textMuted,
                ),
        ...
        actions: [
          ...
          TextButton(
            onPressed: () async {
              Navigator.pop(dCtx);
              try {
                await context.read<ChatProvider>().updateConversation(
                  name: ctrl.text.trim(),
                );
              } catch (e) {
                _snackErr(e);
                    ...
                    children: pinned
                        .map(
                          (m) => ListTile(
                            ...
                            contentPadding: EdgeInsets.zero,
                            title: Text(
                              m.content.isNotEmpty ? m.content : '[Ảnh]',
                              maxLines: 2,
        ...
        actions: [
          IconButton(
            ...
            onPressed: () => Navigator.of(context).push(
              MaterialPageRoute(
                builder: (_) => const ChatSharedContentScreen(),
              ),
            ),
          ),
          IconButton(
  ...
  Widget _inputBar(ChatProvider chat) {
    ...
    return Container(
      ...
      child: archived
          ? Center(
              child: Text(
                'Nhóm đã lưu trữ — không thể gửi tin',
                style: GoogleFonts.inter(
                  fontSize: 13,
                  color: AppColors.textMuted,
                ),
  ...
  Widget _buildBody(ChatProvider chat, String myUserId) {
    ...
    return ListView.builder(
      ...
      itemBuilder: (_, i) {
        ...
        final showSender =
            !isMe && (prev == null || prev.senderMemberId != m.senderMemberId);
