...
class _FamilyMapScreenState extends State<FamilyMapScreen> {
  ...
  Future<void> _routeTo(LatLng target, {String? label}) async {
    ...
    if (from == null) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'Cần vị trí của bạn để chỉ đường — bật GPS rồi thử lại',
          ),
    ...
    final selfDistance = const Distance()
        .as(LengthUnit.Meter, from, target)
        .toDouble();
    if (selfDistance < 50) {
      if (!mounted) return;
      _clearRoute();
    ...
    setState(() {
      ...
      _routeIsRoad = false;
      _routeDistanceM = const Distance()
          .as(LengthUnit.Meter, from, target)
          .toDouble();
  ...
  Widget build(BuildContext context) {
    ...
    final hasAnySos = sosPins.isNotEmpty;
                    ...
                    Expanded(
                      child: Text(
                        pins.where((p) => p.isSos).length == 1
                            ? (sosPins.first.isMe
                                  ? 'Bạn đang trong cảnh báo SOS'
                                  : '${sosPins.first.name} đang cần trợ giúp!')
                            : '${sosPins.length} người đang cần trợ giúp!',
                        style: GoogleFonts.inter(
                          fontSize: 13,
                          fontWeight: FontWeight.w700,
                          color: Colors.white,
  ...
  Widget _routeBanner() {
    ...
    return Container(
      ...
      child: Row(
        children: [
          Icon(
            _routeIsRoad ? Icons.directions_rounded : Icons.timeline_rounded,
            size: 20,
            color: AppColors.link,
          ),
          const SizedBox(width: 10),
    ...
    if (_myPos != null) {
      pins.add(
        _MemberPin(
          ...
          isMe: true,
          isSos: myActiveSos != null,
          alertId: myActiveSos?.id,
                  ...
                  Expanded(
                    child: SizedBox(
                      ...
                      child: ElevatedButton.icon(
                        ...
                        label: Text(
                          pin.isMe ? 'Tôi đã an toàn' : 'Tôi đang đến',
                          style: GoogleFonts.inter(
                            fontSize: 13,
                            fontWeight: FontWeight.w700,
                            color: Colors.white,
  ...
  Future<void> _handleSosPinResponse(_MemberPin pin) async {
    final alertId = pin.alertId;
    ...
    try {
      if (pin.isMe) {
        await sos.confirmSafety(alertId);
        if (canResolveSos) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'Phản hồi SOS thất bại: ${e.toString().replaceFirst('Exception: ', '')}',
          ),
          backgroundColor: AppColors.danger,
        ),
      );
  ...
  Future<void> _resolveSosPin(_MemberPin pin) async {
    final alertId = pin.alertId;
    if (alertId == null) return;
    try {
      await context.read<SosProvider>().resolveAlert(alertId);
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Đã đóng cảnh báo SOS'),
          backgroundColor: AppColors.success,
        ),
      );
    } catch (e) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'Đóng cảnh báo thất bại: ${e.toString().replaceFirst('Exception: ', '')}',
          ),
          backgroundColor: AppColors.danger,
        ),
      );
          ...
          children: [
            Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    'Nhấn nút vị trí bên dưới để hiển thị vị trí của bạn',
                    style: GoogleFonts.inter(
                      fontSize: 13,
                      color: AppColors.textMuted,
                    ),
            ...
            Row(
              children: [
                ...
                Transform.scale(
                  ...
                  child: Switch(
                    value: sharing,
                    onChanged: busy || sharingUnavailable
                        ? null
                        : onToggleSharing,
                    activeThumbColor: AppColors.safe,
                    activeTrackColor: AppColors.safe.withValues(alpha: 0.35),
                  ),
                ),
              ],
            ),
