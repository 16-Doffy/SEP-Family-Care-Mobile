...
class _BudgetPlanDetailScreenState extends State<BudgetPlanDetailScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      context
          ...
          .fetchAll(); // đảm bảo categories/jars sẵn cho form
      _load();
    });
  }
  ...
  Future<void> _load() async {
    ...
    try {
      final (plan, lines) = await context
          .read<FinanceProvider>()
          .fetchBudgetPlanDetail(widget.planId);
      if (mounted)
        setState(() {
          _plan = plan;
          _lines = lines;
              ...
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: Center(
                      child: Text(
                        plan?.planName ?? 'Chi tiết kế hoạch',
                  ...
                  child: ListView(
                    ...
                    children: [
                      ...
                      if (_lines.isEmpty)
                        ...
                        ..._lines.map(
                          (l) => _lineCard(context, l, plan.status == 'DRAFT'),
                        ),
                    ],
                  ),
                ),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  'Kế hoạch: ${_fmt(line.plannedAmount)}${line.actualAmount != null ? ' · Thực tế: ${_fmt(line.actualAmount)}' : ''}',
                  style: GoogleFonts.inter(
                    fontSize: 12,
  ...
  void _showAddLineSheet(BuildContext context) {
    final categories = context
        ...
        .categories
        .where(
          (category) => category.categoryType == 'EXPENSE' && category.isActive,
        )
        .toList();
          ...
          child: Column(
            ...
            children: [
              ...
              if (categories.isEmpty)
                Text(
                  'Chưa có danh mục nào — tạo danh mục trước.',
                  style: GoogleFonts.inter(
                    fontSize: 13,
                    color: AppColors.textMuted,
                  ),
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  onPressed: submitting || categories.isEmpty
                      ? null
                      : () async {
                          ...
                          if (categoryId == null || amt <= 0) {
                            setSheet(
                              () => sheetError =
                                  'Chọn danh mục và nhập số tiền hợp lệ',
                            );
                            return;
                          }
                          setSheet(() {
