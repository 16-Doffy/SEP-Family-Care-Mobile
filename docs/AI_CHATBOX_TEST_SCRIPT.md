...
17. Test kỳ mới: tạo khoản thu và xác nhận trước, ví dụ `Ghi khoản thu
    ...
    “không có quyền” bằng Manager/Deputy hoặc vẫn vượt quỹ sau khi có khoản thu
    cùng kỳ, lưu ảnh để báo BE.
...
16. `Tạo nhiệm vụ đưa con đi học lúc 7h sáng mai, giao cho [tên thành viên]`
    → field trong preview dùng tên **`task`**, kiểm tra hiển thị đúng nội
    dung công việc (không rỗng).
17. Xác nhận → vào danh sách Nhiệm vụ kiểm tra đã tạo đúng người được giao.

## 5. `CREATE_CALENDAR_EVENT`

18. `Tạo lịch họp gia đình cuối tuần này lúc 19h tại nhà` → kiểm tra field
    ngày giờ không hiện chuỗi ISO thô, mà format người đọc được.
19. Xác nhận → vào Lịch kiểm tra sự kiện xuất hiện đúng **tháng của
    `startTime`** (không phải tháng hiện tại nếu event ở tháng sau).

## 6. `ACTION_PLAN_CARD` — kế hoạch nhiều bước

20. `Giúp tôi chuẩn bị cho chuyến du lịch: tạo lịch đi chơi cuối tuần này và
    ghi khoản chi 500.000đ tiền đặt cọc` → phải ra **đúng 1 tin nhắn** (1
    avatar AI) chứa 2 thẻ xếp chồng.
21. Trong thẻ trên: Xác nhận thẻ lịch trước, để thẻ khoản chi ở trạng thái chờ
    → kiểm tra thẻ khoản chi **không bị ảnh hưởng**, vẫn "Chờ xác nhận".
22. Từ chối toàn bộ các bước còn lại trong 1 plan khác → kiểm tra ra đúng 1
    thẻ tổng kết "Kế hoạch đã hủy".

## 7. Phân quyền theo role

23. Đăng nhập bằng **Member** (không phải Manager/Deputy), gửi câu ở mục 1
...
24. Kiểm tra màn Trợ lý AI của Member **không hiện chip gợi ý** tạo khoản
    thu/chi (đã ẩn sẵn ở FE).

## 8. Xác nhận / Từ chối / Hết hạn (outcome màu)

25. Với 1 đề xuất bất kỳ: bấm **Xác nhận** → banner xanh, đúng nội dung kết
    quả (không còn câu "xin vui lòng xác nhận" cũ).
26. Với 1 đề xuất khác: bấm **Hủy đề xuất** → banner **xám**, không rơi về
    thẻ "Phân tích tài chính" chung chung.
27. Chờ một đề xuất hết hạn (hoặc test với `expiresAt` gần) → banner **đỏ**
    "Hết hạn".
28. Bấm nút **"Sửa"** trên 1 thẻ đang chờ → xác nhận hành vi là từ chối đề
...
29. `Rửa bát` (nhiệm vụ đơn giản) + khoản chi 50k — xác nhận/từ chối vẫn ra
    banner đúng màu.
30. Gửi liên tiếp 2-3 câu tạo khoản chi/thu ngẫu nhiên (dùng chip gợi ý random
...
`AI_HANDOFF_LATEST.md` (hiện còn: 4 actionType mục 3 ở trên) — câu nào vẫn
